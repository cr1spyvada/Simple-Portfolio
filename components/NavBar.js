...
export const NavBar = ({ textEnter, textLeave }) => {
  ...
  return (
    ...
    <div className="invisible md:visible fixed w-full md:w-[75vw] left-[12vw] z-[110] top-3 text-2xl md:text-4xl flex justify-between items-center">
      <div className="">Vaisakh</div>
      <button
        // onMouseOver={textEnter}
        // onMouseOut={textLeave}
        onClick={toggleNav}
        className="relative">
        <div className={open ? 'active-hamburger' : 'hamburger'}></div>
      </button>
    </div>
  );
};
